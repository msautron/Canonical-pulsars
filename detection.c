...
int detection(void *params){ //check the flux of each pulsar and if the beam sweps the Earth
	...
	//check_val2=fopen("check_val2.txt","w+");
			...
			if (P_dot_line/part->Pdot[np] >= pow(10,-1.80) && P_dot_line/part->Pdot[np] <= pow(10,1.50)) {
				alpha_l2=70*(M_PI/180)*gsl_rng_uniform(part->r);
				T6_2=(2.8-1.0)*gsl_rng_uniform(part->r)+1.2;
				b2=60*gsl_rng_uniform(part->r)+20;
